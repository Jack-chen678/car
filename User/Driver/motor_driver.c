                       ...
                       unsigned char reverse, int dead_band_speed)
{
    ...
    HAL_GPIO_WritePin(motor->config.in1.port, motor->config.in1.pin, GPIO_PIN_SET);//PE12=1 or PE8=1
    HAL_GPIO_WritePin(motor->config.in2.port, motor->config.in2.pin, GPIO_PIN_RESET);//PE14=0 or PE10=0
...
{
    // 电机速度限幅
    motor->speed = Motor_Limit_Speed(motor, speed, motor->config.htim->Init.Period, -(motor->config.htim->Init.Period));//最大速度:最大占空比 最小速度:-最大占空比
